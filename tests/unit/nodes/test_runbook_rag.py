...
def test_runbook_rag_happy_path(alert, monkeypatch) -> None:
    monkeypatch.setattr(
    ...
    update = runbook_rag({"incident_id": "i1", "alert": alert}, make_config())
    assert update["runbooks"][0].title == "latency-spike-downstream"


def test_runbook_rag_empty_result(alert, monkeypatch) -> None:
    monkeypatch.setattr(
