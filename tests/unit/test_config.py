...
# Hermetic settings builder: point the dotenv source at an empty file so a developer's real
# repo-root `.env` (user-owned, e.g. LLM_PROVIDER=openai) cannot leak into these tests, while
# config.yaml and environment variables still apply. Tests that specifically exercise the .env
# loading path pass their own `_env_file` instead.
def _settings(**kwargs: object) -> Settings:
    return Settings(_env_file=os.devnull, **kwargs)
...
def test_root_settings_ignores_extra_keys() -> None:
    ...
    settings = _settings(bogus_field=1)
    assert settings.llm.provider == "ollama"
