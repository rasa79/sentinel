"""E2E: memory_leak chaos → full investigation through the live API (PLAN.md Task 7.3)."""
...
def test_memory_leak_e2e() -> None:
    inject_chaos(CHAOS_HOST, SERVICE_PORT, "memory_leak", 90)
    wait_for_active_chaos(CHAOS_HOST, SERVICE_PORT, "memory_leak")
    generate_traffic(CHAOS_HOST, SERVICE_PORT)
    incident_id = fire_alert(API, SERVICE, f"memory-leak-{uuid.uuid4().hex[:8]}")
