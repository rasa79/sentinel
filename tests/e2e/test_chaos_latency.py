"""E2E: latency_spike chaos → full investigation through the live API (PLAN.md Task 7.3)."""
...
def test_latency_spike_e2e() -> None:
    inject_chaos(CHAOS_HOST, SERVICE_PORT, "latency_spike", 90)
    wait_for_active_chaos(CHAOS_HOST, SERVICE_PORT, "latency_spike")
    generate_traffic(CHAOS_HOST, SERVICE_PORT)
    incident_id = fire_alert(API, SERVICE, f"latency-spike-{uuid.uuid4().hex[:8]}")
