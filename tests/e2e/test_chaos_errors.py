"""E2E: error_burst chaos → full investigation through the live API (PLAN.md Task 7.3)."""
...
def test_error_burst_e2e() -> None:
    inject_chaos(CHAOS_HOST, SERVICE_PORT, "error_burst", 90)
    wait_for_active_chaos(CHAOS_HOST, SERVICE_PORT, "error_burst")
    generate_traffic(CHAOS_HOST, SERVICE_PORT)
    incident_id = fire_alert(API, SERVICE, f"error-burst-after-deploy-{uuid.uuid4().hex[:8]}")
